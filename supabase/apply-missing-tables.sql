-- ====================================================================
-- VERTEX LAB STUDIO — RUN THIS IN SUPABASE SQL EDITOR
-- Project: https://supabase.com/dashboard/project/swfyalqcopygvzpvlfcl/sql
-- ====================================================================

-- 1. PROBLEM TRACKER LEADS TABLE
...
drop policy if exists "portfolio_public_read" on portfolio_projects;
create policy "portfolio_public_read" on portfolio_projects 
  for select using (active = true or is_admin());

drop policy if exists "portfolio_admin_write" on portfolio_projects;
create policy "portfolio_admin_write" on portfolio_projects 
  for insert with check (is_admin());

drop policy if exists "portfolio_admin_update" on portfolio_projects;
create policy "portfolio_admin_update" on portfolio_projects 
  for update using (is_admin());

drop policy if exists "portfolio_admin_delete" on portfolio_projects;
create policy "portfolio_admin_delete" on portfolio_projects 
  for delete using (is_admin());

-- Seed initial portfolio case studies
...
-- 3. SEED SETTINGS (Booking & Social Proof Defaults)
insert into site_settings (key, value)
values
(
  'booking',
  '{"url": "https://cal.com/vertex-lab-studio/discovery", "label": "Book a 15-min discovery call", "enabled": true}'::jsonb
),
(
  'social_proof',
  '{"projectsDelivered": "48+", "uptimeGuarantee": "99.9%", "avgSprintWeeks": "2-3 wks", "clientRetention": "100%"}'::jsonb
)
